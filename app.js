const $=s=>document.querySelector(s),S={q:[],sub:'All'};function p(t){let y,g,o=[];for(let b of t.replace(/\r/g,'').split(/\n\s*\n/)){let m=b.match(/^YEAR:\s*(\d{4})/m);if(m){y=+m[1];continue}m=b.match(/^\[([^|\]]+)\s*\|\s*Unit:\s*([^|\]]+)\s*\|\s*Topic:\s*([^\]]+)\]/m);if(m)g={subject:m[1].trim(),unit:m[2].trim(),topic:m[3].trim()};let q=b.match(/Q\.(\d+)\.\s*([\s\S]*?)(?=\nA\.\s)/),a=[...b.matchAll(/^([A-E])\.\s*(.*)$/gm)];if(q&&g&&y&&a.length>1)o.push({year:y,...g,stem:q[2].trim(),a})}return o}function esc(s){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
let answers={};try{answers=JSON.parse(localStorage.getItem('myelin-answers')||'{}')}catch{}
function key(x){return x.year+':'+x.stem}
function show(q,t,k){
$('#question-kicker').textContent=k;
$('#question-title').textContent=t;
function progress(){$('#question-count').textContent=q.filter(x=>answers[key(x)]).length+' / '+q.length+' attempted · Answer key not supplied; scoring unavailable. Tags are supplied, not independently verified.'}
progress();
$('#question-list').innerHTML=q.map((x,i)=>'<article class="question"><div class="question-meta">'+esc(x.year+' · '+x.subject+' · '+x.unit+' · '+x.topic)+'</div><div class="question-stem">'+(i+1)+'. '+esc(x.stem)+'</div><div class="options" role="group" aria-label="Answers for question '+(i+1)+'">'+x.a.map(a=>'<button type="button" class="option" style="text-align:left;cursor:pointer" data-q="'+i+'" data-answer="'+a[1]+'" aria-pressed="'+(answers[key(x)]===a[1])+'"><b>'+a[1]+'.</b>'+esc(a[2])+'</button>').join('')+'</div></article>').join('');
function paint(){document.querySelectorAll('[data-answer]').forEach(b=>{const selected=answers[key(q[Number(b.dataset.q)])]===b.dataset.answer;b.setAttribute('aria-pressed',String(selected));b.style.background=selected?'#dbeafe':'#f8fafc';b.style.borderColor=selected?'#2563eb':'#e5eaf1'})}
